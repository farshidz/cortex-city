...
Cortex City uses one reviewer agent for both PRs created by Cortex tasks and other people's PRs that you are assigned to review. Task-owned reviews use the task's title, description, and plan as the scope authority, and leave actionable GitHub comments for changes the implementation agent should make. Every required-change comment states its scope basis and minimum safe fix. A blocking finding does not authorize the reviewer to prescribe a scope-expanding redesign: when safety would require new durable state, protocols, cross-store guarantees, or other architectural work, the reviewer asks for a human scope decision instead of assigning that implementation to the builder. Broader improvements remain non-blocking suggestions for a separate task and PR. Turn off `Automatic review` on a task when you do not want its PR added to this review flow.
