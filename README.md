...
For someone else's PR, a clean review automatically approves the current head. When the reviewer needs a human decision, it posts the uncertain or advisory points on the PR. Self-authored and task-owned PRs are never approved by the reviewer because GitHub does not allow authors to approve their own changes.
