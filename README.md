...
Reviewer handoff comments are immutable timeline events. Cortex City saves each delivery action before posting, verifies the GitHub author and exact body before recording its receipt, and recovers interrupted posts without editing, deleting, or duplicating the event. Only IDs from those verified receipts are excluded from task feedback detection, so copied reviewer prefixes and markers remain ordinary participant comments.
