...
Settings also provides `Review Author Whitelist` (`review_author_whitelist`, default `[]`) and `Weekly Codex Usage Limit (%)` (`review_weekly_usage_limit_percent`, default `20`, integer 0–100). An empty whitelist disables the gate. Otherwise, authors on the list are exempt (case-insensitive GitHub logins), and reviews for other authors are refused when account-wide Codex weekly usage reaches the threshold. This applies to scheduled and manually regenerated reviews, regardless of the reviewer runtime. The weekly window is identified by its 10,080-minute duration in the main `codex` rate-limit bucket; model-specific buckets are excluded. Usage is cached for up to 60 seconds. Missing author or usage data defers the review until it can be checked. A refused PR gets one reviewer comment saying the weekly Codex usage limit has been exceeded. Confirmed comments are cached for later polls; after a restart, compact page-by-page discovery recovers an existing notice. Manual regeneration displays any deferral error on the review page. Polling retries automatically, so reviews can resume after usage resets or configuration changes.
