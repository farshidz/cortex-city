...
Cortex City uses one reviewer agent for both PRs created by Cortex tasks and other people's PRs that you are assigned to review. Task-owned reviews use the task's title, description, and plan as the scope authority; other reviews use the PR's stated goal and the repository's existing contracts. The reviewer reports real defects but requires only the smallest correct in-scope fix. If a blocking issue has no small in-scope remedy, it asks for a human scope decision instead of assigning a larger design to the author. Broader improvements remain non-blocking suggestions for a separate task and PR. Turn off `Automatic review` on a task when you do not want its PR added to this review flow.

For someone else's PR, a clean review automatically approves the exact reviewed commit. When the reviewer needs a human decision, it posts the uncertain or advisory points as a new top-level PR comment. Self-authored and task-owned PRs are never approved by the reviewer because GitHub does not allow authors to approve their own changes; a clean review instead posts a handoff requesting an eligible non-author reviewer or another repository-permitted manual action.

Reviewer comments are immutable timeline events: follow-up agents post new comments instead of editing or deleting earlier findings or handoffs. Cortex City fsyncs each handoff delivery action before posting, serializes recovery and POST attempts across processes, verifies the GitHub author and exact body before durably recording its receipt, and recovers interrupted posts without duplicating the event. Before a new POST it also verifies that the PR is open at the reviewed SHA; an undelivered stale action is durably canceled, while an exact comment already accepted by GitHub is still recovered and receipted. Every comment the reviewer posts is receipted per surface (PR conversation or inline review) and excluded from task feedback detection and PR-state hashes, so the reviewer's own findings, replies, and summaries never wake a builder or register as new conversation. The two handoff comments are receipted as verified immutable events, matched by author and exact body against a durable action token. Comments a run posted itself are matched by author plus the public reviewer prefix anchored at the start of the body, which is the fallback for IDs a run failed to record. A copied prefix from any other author remains an ordinary participant comment. One residual case is accepted: on a self-authored PR the human author and the reviewer share a GitHub login, so human-written content that literally begins with the reviewer prefix is treated as reviewer-authored. That heuristic now decides more than hash suppression. GitHub carries each inline comment in a review of its own and sometimes leaves that review unsubmitted; an unsubmitted (`PENDING`) review is visible only to its author, GitHub allows one per user per PR, and while it stands every later inline comment joins it instead of publishing. After each round, and again from the worker poll for as long as the condition is recorded, Cortex City repairs its own draft: it submits one whose every held comment **and** review body carry the reviewer prefix — passing an existing body back byte-for-byte, since submitting replaces it — deletes one holding no comments and no body, and leaves any other draft untouched. So on a shared login, human content beginning with that prefix can be submitted as the reviewer's, and a draft holding nothing can be deleted; both are irreversible. A draft holding content Cortex City did not author is never submitted or deleted, but it does block reviewer comments until its author acts, and discarding it discards any reviewer comments it holds — a re-review is what reproduces them. An unrepaired condition is recorded in `pending_review_error` and shown on the review, covering a foreign comment or body, a failed comment listing, a failed empty-draft delete, and a failed submit. Exact per-run provenance would require a new prompt-to-filter protocol and is deliberately not built.
...
Settings also provides `Authors Exempt from Usage Limit` (`review_author_whitelist`, default `[]`) and `Weekly Codex Usage Limit (%)` (`review_weekly_usage_limit_percent`, unset by default, integer 0–100). An unset limit disables the gate regardless of the author list. With a limit configured, authors on the list are exempt (case-insensitive GitHub logins), and reviews for everyone else are refused when account-wide Codex weekly usage reaches the threshold. An empty list applies the limit to everyone. Clearing the limit in Settings or sending `null` or an empty string through the config API disables it; `0` is an active limit. Existing stored numeric limits remain configured. This applies to scheduled and manually regenerated reviews, regardless of the reviewer runtime. The weekly window is identified by its 10,080-minute duration in the main `codex` rate-limit bucket; model-specific buckets are excluded. Usage is cached for up to 60 seconds. When the gate is active, unavailable usage data defers the review. Author lookup is required only when the exemption list is populated; unavailable author data then also defers the review. A refused PR gets one reviewer comment saying the weekly Codex usage limit has been exceeded. Confirmed comments are cached for later polls; after a restart, compact page-by-page discovery recovers an existing notice. Manual regeneration displays any deferral error on the review page. Polling retries automatically, so reviews can resume after usage resets or configuration changes.

Each PR gets an isolated directory under `tmp/reviews/` that is reused by its reviews, follow-ups, and retrospective. Cortex City removes the directory after the review is finalized and no review or retrospective process is active. The worker reconciles this cleanup on later polls, so a missed merge or close event does not leave terminal-review artifacts behind. Set `CORTEX_REVIEW_WORKSPACE_ROOT` in the web and worker service environments to place these workspaces elsewhere.
