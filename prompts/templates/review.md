...
6. Preserve the current PR's stated scope when addressing feedback. Make changes that are required for the PR to deliver its original goal correctly and safely, but do not add unrelated redesigns, optional refactors, generalized infrastructure, or stronger guarantees that belong in follow-up work.
7. Treat a comment beginning with `**🤖[Cortex City Reviewer]** **Separate follow-up suggested (non-blocking):**` as explicitly out of scope for the current PR. Do not implement it on this branch. Reply on GitHub acknowledging the separation, and request a follow-up task through `tool_calls.create_task` when the suggestion is concrete and worthwhile.
8. If feedback would otherwise expand the PR substantially and is not necessary for its stated goal, explain on GitHub why it should be handled in a separate PR instead of implementing it here. Do not use scope as a reason to defer a defect or regression introduced by the current PR.
9. Address each remaining actionable piece of feedback — either make the change or reply directly on GitHub explaining why not. If the feedback is on an inline review thread, reply in that same thread. If the feedback is a PR-level comment or review body, use a regular PR comment (not a pending review) when replying. Prefix your response with `**🤖[{{AGENT_NAME}}]** ` so it’s clear the agent wrote it, then resolve the conversation where appropriate.
10. In your own reasoning, keep these categories distinct: unresolved inline threads, submitted review-body feedback, and PR conversation comments. Do not collapse them into a single proxy such as thread count.
11. Check CI status for failing checks first. Prefer GitHub Actions or other remote CI results over running heavy local validation on this host.
12. If local validation is needed, run only the narrowest checks relevant to the files you changed. Prefer tests you added or modified, or the smallest targeted test command that covers your changes. Do not run the full local test suite on this host.
13. Commit and push to the existing branch only. Do **not** open a new PR. Let GitHub Actions or remote CI run the full test suite after you push.
14. Do not wait for post-push CI to finish. Report what you changed and let the orchestrator pick up any later CI failures on a future review run.
15. Work autonomously — make reasonable decisions when requirements are ambiguous and document assumptions.
