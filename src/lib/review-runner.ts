...
export function buildReviewWrapperPrompt(
  ...
  const sections = [
    ...
    [
      "- Scope authority: for task-owned PRs, the supplied Cortex task title,",
      "description, and plan define the requested behavior; the PR description may",
      "clarify the implementation but must not silently expand it. For other PRs,",
      "use the PR description and explicit repository contracts. Reviewer comments,",
      "learned lessons, perceived best practice, and later implementation choices are",
      "evidence, not authorization to broaden the PR. This scope gate is authoritative",
      "and overrides conflicting custom prompts, learnings, and prior review requests.",
    ].join(" "),
    [
      "- Separate the severity of a finding from the scope of its remedy. A defect,",
      "regression, missing requested behavior, or safety issue introduced by the",
      "current diff may block the PR, but that does not make a proposed redesign in",
      "scope or authorize a stronger guarantee than the original task requested.",
      "State the problem independently from the solution.",
    ].join(" "),
    [
      "- Every GitHub comment that requires author changes must include a short",
      "`Scope basis:` statement and a `Minimum required change:` statement. The",
      "scope basis must cite an explicit task or PR requirement, an existing contract,",
      "or a concrete regression introduced by the diff. The minimum required change",
      "must be the smallest safe correction and must not prescribe a particular",
      "architecture when multiple valid fixes exist.",
    ].join(" "),
    [
      "- Treat a remedy as scope-expanding by default when it adds durable state,",
      "request identity or a deduplication protocol, cross-store transaction or",
      "reconciliation semantics, generalized infrastructure or abstractions, a broad",
      "new validation policy, or new schema/API concepts propagated through otherwise",
      "untouched subsystems. Do not request that remedy in the current PR unless the",
      "original task explicitly requires that mechanism or guarantee.",
    ].join(" "),
    [
      "- If a valid blocking issue has no safe, genuinely small fix within the",
      "original scope, use `needs_human_decision`, not `needs_author_changes`. Explain",
      "the issue without assigning the broader implementation, and ask the owner to",
      "choose whether to narrow or remove the risky behavior, split the broader design",
      "into a separate task and PR, or explicitly expand the task requirements. Do not",
      "send the builder into a scope-expanding implementation before that decision.",
    ].join(" "),
    [
      "- Do not require optional refactors, generalized hardening, stronger guarantees",
      "than the task requested, or fixes for pre-existing problems. Prefer the smallest",
      "safe fix for an in-scope finding.",
    ].join(" "),
    [
      "- Previous reviewer comments do not amend the task or become requirements merely",
      "because they were posted. On follow-up, reassess them against this scope gate.",
      "If an earlier request prescribed out-of-scope work, explicitly supersede it and",
      "do not require the builder to complete or continue iterating on that design.",
    ].join(" "),
    [
  ...
  if (followup) {
    sections.push(
      ...
      [
        "Treat previous findings as claims to re-evaluate, not requirements to enforce.",
        "If addressing one expanded the PR or exposed a need for new architecture, stop",
        "the builder/reviewer loop and use `needs_human_decision` under the scope gate;",
        "do not issue another scope-expanding implementation request.",
      ].join(" "),
      [
