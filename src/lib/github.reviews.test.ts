...
test("getMyReviewSignals prefers a newer same-second change request by review ID", () => {
  const workspace = setupWorkspace();
  const prUrl = "https://github.com/acme/widget/pull/1";
  ...
  assert.deepEqual(result, {
    last_review_sha: "sha1",
    changes_requested_sha: "sha1",
  });
});

test("getMyReviewSignals prefers a newer same-second approval by review ID", () => {
  const workspace = setupWorkspace();
  const prUrl = "https://github.com/acme/widget/pull/1";
  const submittedAt = "2026-05-01T00:00:00Z";
  const responses: Record<string, FakeGhResponse> = {
